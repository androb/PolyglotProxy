    ...
    function openInsertBlogDialog() {
      return editor.windowManager.open({
        ...
        onSubmit: async function(api) {
          ...
          api.close();
          insertContent(`Create a blog post about ${data.topic}. Write it in a ${data.tone} tone. Use transition words. Use active voice. Write ${data.wordCount} words. Add title and subtitle for each section. Keep each paragrah short. Include the following keywords: “${data.keywords}”. Use HTML to format headings, paragraphs, and lists. Use HTML to output the content.`, 2000);
        },
      });
    }
    ...
    function openInsertImageDialog() {
      return editor.windowManager.open({
          ...
          items: [
            ...
            },
            {
              ...
              items: [
                ...
                { text: 'Abstract Expressionism', value: 'abstract expressionism' },
                { text: 'Anime', value: 'anime' },
                { text: 'Baroque', value: 'baroque' },
                { text: 'Blue Print', value: 'blue print' },
                { text: 'Cartoon', value: 'cartoon' },
                { text: 'Children\'s Illustration', value: 'children\'s illustration' },
                ...
                { text: 'Cyberpunk Illustration', value: 'cyberpunk illustration' },
        ...
        onSubmit: async function(api) {
          ...
          editor.insertContent(`<figure class="image"><img src="${imageURL}" alt="${imagePrompt}" /><figcaption>"${imagePrompt}" by DALL·E 2</figcaption></figure>`);
        },
      });
    }
    ...
    function openInsertPoemDialog() {
      return editor.windowManager.open({
        ...
        onSubmit: async function(api) {
          ...
          editor.setProgressState(true);
          try {
            const poem = await generatePoem(data.topic, data.style, data.wordCount);
    ...
    var insertActions = [
      ...
      },
      {
        ...
        action: async function() {
          editor.setProgressState(true);
          try {
            const summary = await generateSummary(editor.getContent({ format: 'text' }));
